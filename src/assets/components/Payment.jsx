import { useState, useEffect, useRef } from "react";
import SubNav from "./SubNav";
import YuchoVideoModal from "./YuchoVideoModal";
import { useParams } from "react-router-dom";
import { course1 } from "../../data/course1";
import { BasicJapanese } from "../../data/BasicJapanese";

function Payment() {
  const { id } = useParams();
  const planData =
    course1.find((plan) => plan.id === Number(id)) ||
    BasicJapanese.find((plan) => plan.id === Number(id));
  const specialCardIds = [7, 8, 9, 10, 11, 12];
  const isSpecialCardDesign = specialCardIds.includes(Number(id));

  const isGroupA = [7, 8].includes(Number(id));
  const isGroupB = [9, 10].includes(Number(id));
  const isGroupC = [11, 12].includes(Number(id));

  const [method, setMethod] = useState("card");
  const initialized = useRef(false);
  const cardRef = useRef(null);
  const cardContainerRef = useRef(null);
  const cardButtonRef = useRef(null);
  const [showModal, setShowModal] = useState(false);
  const [agreed, setAgreed] = useState(false);

  const getDisplayPrice = () => {
    // Remove all non-digit characters
    const basePrice = Number(planData.price.toString().replace(/\D/g, ""));
    let extra = 0;
    if (method === "bank") extra = 1000;
    if (method === "paypal") extra = 1500;
    return basePrice + extra;
  };

  useEffect(() => {
    if (method !== "card") {
      initialized.current = false;
      if (cardRef.current) {
        cardRef.current.destroy();
        cardRef.current = null;
      }
      return;
    }

    if (initialized.current) return;
    async function initializeSquare() {
      if (!window.Square) return;
      if (!cardContainerRef.current) return;

      initialized.current = true;

      // Clear old content
      cardContainerRef.current.innerHTML = "";

      const payments = window.Square.payments(
        "sandbox-sq0idb-GT3CpIXo9vTmJ2_F4V1M_Q",
        "LQJGB7GYRAA04",
      );

      const card = await payments.card();

      cardRef.current = card;

      await card.attach(cardContainerRef.current);

      cardButtonRef.current.onclick = async () => {
        if (!agreed) {
          setShowModal(true);
          return;
        }
        const result = await card.tokenize();

        if (result.status === "OK") {
          console.log("Token:", result.token);
          alert("Payment token created!");
        } else {
          console.error(result.errors);
        }
      };
    }

    initializeSquare();
    return () => {
      if (cardRef.current) {
        cardRef.current.destroy();
        cardRef.current = null;
      }
    };
  }, [method, agreed]);

  return (
    <>
      <SubNav
        items={[{ label: "ホームページ", link: "/" }, { label: "お支払い" }]}
      />

      <div className="container checkout">
        <div className="head-title">お支払い</div>

        <div className="row">
          {/* LEFT SIDE */}
          <div className="col-md-6 set-info">
            <div className="name">{planData.title}</div>

            <div className="price">
              {isSpecialCardDesign ? (
                <>
                  ￥{getDisplayPrice().toLocaleString()}円/
                  <small>{planData.duration}</small>
                </>
              ) : (
                <>
                  {getDisplayPrice().toLocaleString()}円/
                  <small className="fs-6">{planData.duration}</small>
                </>
              )}
            </div>

            <div
              className="description"
              dangerouslySetInnerHTML={{
                __html: planData.description,
              }}
            ></div>
          </div>

          {/* RIGHT SIDE */}
          <div className="col-md-6 set-info">
            <div className="payment">
              <div className="text-lg mb-3">{planData.paymentTitle}</div>

              <div className="payment-type mb-4">
                {planData.paymentMethods.includes("card") && (
                  <label>
                    <input
                      type="radio"
                      checked={method === "card"}
                      onChange={() => setMethod("card")}
                    />
                    {planData.paymentMethodsLabels?.card}
                  </label>
                )}

                {planData.paymentMethods.includes("bank") && (
                  <label>
                    <input
                      type="radio"
                      checked={method === "bank"}
                      onChange={() => setMethod("bank")}
                    />
                    {planData.paymentMethodsLabels?.bank}
                  </label>
                )}

                {planData.paymentMethods.includes("Paypal") && (
                  <label>
                    <input
                      type="radio"
                      checked={method === "paypal"}
                      onChange={() => setMethod("paypal")}
                    />
                    {planData.paymentMethodsLabels?.paypal}
                  </label>
                )}
              </div>

              <div className="content">
                {method === "card" && (
                  <>
                    {
                      // isSpecialCardDesign ? (
                      //   <div className="special-card-design">
                      //     <button className="paypal-btn">
                      //       <svg
                      //         width="80px"
                      //         height="25"
                      //         viewBox="0 0 101 32"
                      //         preserveAspectRatio="xMinYMin meet"
                      //         xmlns="http:&#x2F;&#x2F;www.w3.org&#x2F;2000&#x2F;svg"
                      //       >
                      //         <path
                      //           fill="#003087"
                      //           d="M 12.237 2.8 L 4.437 2.8 C 3.937 2.8 3.437 3.2 3.337 3.7 L 0.237 23.7 C 0.137 24.1 0.437 24.4 0.837 24.4 L 4.537 24.4 C 5.037 24.4 5.537 24 5.637 23.5 L 6.437 18.1 C 6.537 17.6 6.937 17.2 7.537 17.2 L 10.037 17.2 C 15.137 17.2 18.137 14.7 18.937 9.8 C 19.237 7.7 18.937 6 17.937 4.8 C 16.837 3.5 14.837 2.8 12.237 2.8 Z M 13.137 10.1 C 12.737 12.9 10.537 12.9 8.537 12.9 L 7.337 12.9 L 8.137 7.7 C 8.137 7.4 8.437 7.2 8.737 7.2 L 9.237 7.2 C 10.637 7.2 11.937 7.2 12.637 8 C 13.137 8.4 13.337 9.1 13.137 10.1 Z"
                      //         ></path>
                      //         <path
                      //           fill="#003087"
                      //           d="M 35.437 10 L 31.737 10 C 31.437 10 31.137 10.2 31.137 10.5 L 30.937 11.5 L 30.637 11.1 C 29.837 9.9 28.037 9.5 26.237 9.5 C 22.137 9.5 18.637 12.6 17.937 17 C 17.537 19.2 18.037 21.3 19.337 22.7 C 20.437 24 22.137 24.6 24.037 24.6 C 27.337 24.6 29.237 22.5 29.237 22.5 L 29.037 23.5 C 28.937 23.9 29.237 24.3 29.637 24.3 L 33.037 24.3 C 33.537 24.3 34.037 23.9 34.137 23.4 L 36.137 10.6 C 36.237 10.4 35.837 10 35.437 10 Z M 30.337 17.2 C 29.937 19.3 28.337 20.8 26.137 20.8 C 25.037 20.8 24.237 20.5 23.637 19.8 C 23.037 19.1 22.837 18.2 23.037 17.2 C 23.337 15.1 25.137 13.6 27.237 13.6 C 28.337 13.6 29.137 14 29.737 14.6 C 30.237 15.3 30.437 16.2 30.337 17.2 Z"
                      //         ></path>
                      //         <path
                      //           fill="#003087"
                      //           d="M 55.337 10 L 51.637 10 C 51.237 10 50.937 10.2 50.737 10.5 L 45.537 18.1 L 43.337 10.8 C 43.237 10.3 42.737 10 42.337 10 L 38.637 10 C 38.237 10 37.837 10.4 38.037 10.9 L 42.137 23 L 38.237 28.4 C 37.937 28.8 38.237 29.4 38.737 29.4 L 42.437 29.4 C 42.837 29.4 43.137 29.2 43.337 28.9 L 55.837 10.9 C 56.137 10.6 55.837 10 55.337 10 Z"
                      //         ></path>
                      //         <path
                      //           fill="#009cde"
                      //           d="M 67.737 2.8 L 59.937 2.8 C 59.437 2.8 58.937 3.2 58.837 3.7 L 55.737 23.6 C 55.637 24 55.937 24.3 56.337 24.3 L 60.337 24.3 C 60.737 24.3 61.037 24 61.037 23.7 L 61.937 18 C 62.037 17.5 62.437 17.1 63.037 17.1 L 65.537 17.1 C 70.637 17.1 73.637 14.6 74.437 9.7 C 74.737 7.6 74.437 5.9 73.437 4.7 C 72.237 3.5 70.337 2.8 67.737 2.8 Z M 68.637 10.1 C 68.237 12.9 66.037 12.9 64.037 12.9 L 62.837 12.9 L 63.637 7.7 C 63.637 7.4 63.937 7.2 64.237 7.2 L 64.737 7.2 C 66.137 7.2 67.437 7.2 68.137 8 C 68.637 8.4 68.737 9.1 68.637 10.1 Z"
                      //         ></path>
                      //         <path
                      //           fill="#009cde"
                      //           d="M 90.937 10 L 87.237 10 C 86.937 10 86.637 10.2 86.637 10.5 L 86.437 11.5 L 86.137 11.1 C 85.337 9.9 83.537 9.5 81.737 9.5 C 77.637 9.5 74.137 12.6 73.437 17 C 73.037 19.2 73.537 21.3 74.837 22.7 C 75.937 24 77.637 24.6 79.537 24.6 C 82.837 24.6 84.737 22.5 84.737 22.5 L 84.537 23.5 C 84.437 23.9 84.737 24.3 85.137 24.3 L 88.537 24.3 C 89.037 24.3 89.537 23.9 89.637 23.4 L 91.637 10.6 C 91.637 10.4 91.337 10 90.937 10 Z M 85.737 17.2 C 85.337 19.3 83.737 20.8 81.537 20.8 C 80.437 20.8 79.637 20.5 79.037 19.8 C 78.437 19.1 78.237 18.2 78.437 17.2 C 78.737 15.1 80.537 13.6 82.637 13.6 C 83.737 13.6 84.537 14 85.137 14.6 C 85.737 15.3 85.937 16.2 85.737 17.2 Z"
                      //         ></path>
                      //         <path
                      //           fill="#009cde"
                      //           d="M 95.337 3.3 L 92.137 23.6 C 92.037 24 92.337 24.3 92.737 24.3 L 95.937 24.3 C 96.437 24.3 96.937 23.9 97.037 23.4 L 100.237 3.5 C 100.337 3.1 100.037 2.8 99.637 2.8 L 96.037 2.8 C 95.637 2.8 95.437 3 95.337 3.3 Z"
                      //         ></path>
                      //       </svg>
                      //       <span className="fs-5 text-body ms-1">Checkout</span>
                      //     </button>
                      //     <button className="card-btn">
                      //       <span>
                      //         <svg
                      //           width="25px"
                      //           height="20px"
                      //           viewBox="0 0 24 18"
                      //           xmlns="http:&#x2F;&#x2F;www.w3.org&#x2F;2000&#x2F;svg"
                      //         >
                      //           <g
                      //             stroke="none"
                      //             stroke-width="1"
                      //             fill="none"
                      //             fill-rule="evenodd"
                      //           >
                      //             <g
                      //               transform="translate(-3.000000, -6.000000)"
                      //               fill="#ffffff"
                      //               fill-rule="nonzero"
                      //             >
                      //               <path
                      //                 d="M8.27521338,12.5122653 C7.93003542,12.5122653 7.65021338,12.2324432 7.65021338,11.8872653 C7.65021338,11.5420873 7.93003542,11.2622653 8.27521338,11.2622653 L24.7879042,11.2622653 C25.5955939,11.2622653 26.25,11.9175905 26.25,12.7255368 L26.25,22.2867284 C26.25,23.0946748 25.5955939,23.75 24.7879042,23.75 L5.21231302,23.75 C4.40462325,23.75 3.75,23.0946748 3.75,22.2867397 L3.75,7.71327152 C3.75,6.90532518 4.40440608,6.25 5.21227212,6.25 L24.7880664,6.25552163 C25.5956079,6.25573147 26.25,6.91099507 26.25,7.71870362 L26.25,9.23577161 C26.25,9.58094958 25.9702675,9.86081168 25.6250895,9.86086112 C25.2799115,9.86091055 25.0000494,9.5811286 25,9.23595063 L24.9997827,7.71879313 C24.9997827,7.60083189 24.9046611,7.50555197 24.7877278,7.50552158 L5.21209583,7.49999998 C5.09515506,7.49999998 5,7.59528868 5,7.71326028 L5.00021718,22.2867284 C5.00021718,22.4047113 5.09537223,22.5 5.21231302,22.5 L24.7879042,22.5 C24.904845,22.5 25,22.4047113 25,22.2867284 L25,12.7255368 C25,12.607554 24.9048449,12.5122653 24.7879042,12.5122653 L8.27521338,12.5122653 Z"
                      //                 id="Stroke-1"
                      //               ></path>
                      //             </g>
                      //           </g>
                      //         </svg>
                      //       </span>
                      //       <span className="fs-5">Debit or Credit Card</span>
                      //     </button>
                      //     <div className="d-flex justify-content-center mt-2">
                      //       <i
                      //         className="text-muted"
                      //         style={{ fontSize: "10px" }}
                      //       >
                      //         Powered By&nbsp;
                      //       </i>
                      //       <svg
                      //         width="50px"
                      //         height="25"
                      //         viewBox="0 0 101 32"
                      //         preserveAspectRatio="xMinYMin meet"
                      //         xmlns="http:&#x2F;&#x2F;www.w3.org&#x2F;2000&#x2F;svg"
                      //       >
                      //         <path
                      //           fill="#003087"
                      //           d="M 12.237 2.8 L 4.437 2.8 C 3.937 2.8 3.437 3.2 3.337 3.7 L 0.237 23.7 C 0.137 24.1 0.437 24.4 0.837 24.4 L 4.537 24.4 C 5.037 24.4 5.537 24 5.637 23.5 L 6.437 18.1 C 6.537 17.6 6.937 17.2 7.537 17.2 L 10.037 17.2 C 15.137 17.2 18.137 14.7 18.937 9.8 C 19.237 7.7 18.937 6 17.937 4.8 C 16.837 3.5 14.837 2.8 12.237 2.8 Z M 13.137 10.1 C 12.737 12.9 10.537 12.9 8.537 12.9 L 7.337 12.9 L 8.137 7.7 C 8.137 7.4 8.437 7.2 8.737 7.2 L 9.237 7.2 C 10.637 7.2 11.937 7.2 12.637 8 C 13.137 8.4 13.337 9.1 13.137 10.1 Z"
                      //         ></path>
                      //         <path
                      //           fill="#003087"
                      //           d="M 35.437 10 L 31.737 10 C 31.437 10 31.137 10.2 31.137 10.5 L 30.937 11.5 L 30.637 11.1 C 29.837 9.9 28.037 9.5 26.237 9.5 C 22.137 9.5 18.637 12.6 17.937 17 C 17.537 19.2 18.037 21.3 19.337 22.7 C 20.437 24 22.137 24.6 24.037 24.6 C 27.337 24.6 29.237 22.5 29.237 22.5 L 29.037 23.5 C 28.937 23.9 29.237 24.3 29.637 24.3 L 33.037 24.3 C 33.537 24.3 34.037 23.9 34.137 23.4 L 36.137 10.6 C 36.237 10.4 35.837 10 35.437 10 Z M 30.337 17.2 C 29.937 19.3 28.337 20.8 26.137 20.8 C 25.037 20.8 24.237 20.5 23.637 19.8 C 23.037 19.1 22.837 18.2 23.037 17.2 C 23.337 15.1 25.137 13.6 27.237 13.6 C 28.337 13.6 29.137 14 29.737 14.6 C 30.237 15.3 30.437 16.2 30.337 17.2 Z"
                      //         ></path>
                      //         <path
                      //           fill="#003087"
                      //           d="M 55.337 10 L 51.637 10 C 51.237 10 50.937 10.2 50.737 10.5 L 45.537 18.1 L 43.337 10.8 C 43.237 10.3 42.737 10 42.337 10 L 38.637 10 C 38.237 10 37.837 10.4 38.037 10.9 L 42.137 23 L 38.237 28.4 C 37.937 28.8 38.237 29.4 38.737 29.4 L 42.437 29.4 C 42.837 29.4 43.137 29.2 43.337 28.9 L 55.837 10.9 C 56.137 10.6 55.837 10 55.337 10 Z"
                      //         ></path>
                      //         <path
                      //           fill="#009cde"
                      //           d="M 67.737 2.8 L 59.937 2.8 C 59.437 2.8 58.937 3.2 58.837 3.7 L 55.737 23.6 C 55.637 24 55.937 24.3 56.337 24.3 L 60.337 24.3 C 60.737 24.3 61.037 24 61.037 23.7 L 61.937 18 C 62.037 17.5 62.437 17.1 63.037 17.1 L 65.537 17.1 C 70.637 17.1 73.637 14.6 74.437 9.7 C 74.737 7.6 74.437 5.9 73.437 4.7 C 72.237 3.5 70.337 2.8 67.737 2.8 Z M 68.637 10.1 C 68.237 12.9 66.037 12.9 64.037 12.9 L 62.837 12.9 L 63.637 7.7 C 63.637 7.4 63.937 7.2 64.237 7.2 L 64.737 7.2 C 66.137 7.2 67.437 7.2 68.137 8 C 68.637 8.4 68.737 9.1 68.637 10.1 Z"
                      //         ></path>
                      //         <path
                      //           fill="#009cde"
                      //           d="M 90.937 10 L 87.237 10 C 86.937 10 86.637 10.2 86.637 10.5 L 86.437 11.5 L 86.137 11.1 C 85.337 9.9 83.537 9.5 81.737 9.5 C 77.637 9.5 74.137 12.6 73.437 17 C 73.037 19.2 73.537 21.3 74.837 22.7 C 75.937 24 77.637 24.6 79.537 24.6 C 82.837 24.6 84.737 22.5 84.737 22.5 L 84.537 23.5 C 84.437 23.9 84.737 24.3 85.137 24.3 L 88.537 24.3 C 89.037 24.3 89.537 23.9 89.637 23.4 L 91.637 10.6 C 91.637 10.4 91.337 10 90.937 10 Z M 85.737 17.2 C 85.337 19.3 83.737 20.8 81.537 20.8 C 80.437 20.8 79.637 20.5 79.037 19.8 C 78.437 19.1 78.237 18.2 78.437 17.2 C 78.737 15.1 80.537 13.6 82.637 13.6 C 83.737 13.6 84.537 14 85.137 14.6 C 85.737 15.3 85.937 16.2 85.737 17.2 Z"
                      //         ></path>
                      //         <path
                      //           fill="#009cde"
                      //           d="M 95.337 3.3 L 92.137 23.6 C 92.037 24 92.337 24.3 92.737 24.3 L 95.937 24.3 C 96.437 24.3 96.937 23.9 97.037 23.4 L 100.237 3.5 C 100.337 3.1 100.037 2.8 99.637 2.8 L 96.037 2.8 C 95.637 2.8 95.437 3 95.337 3.3 Z"
                      //         ></path>
                      //       </svg>
                      //     </div>
                      //   </div>
                      // ) : (
                      <div className="card-box-wrapper">
                        {!agreed && (
                          <div
                            className="card-overlay"
                            onClick={() => setShowModal(true)}
                          />
                        )}
                        <div className="card-box">
                          <div ref={cardContainerRef}></div>
                        </div>
                        <button ref={cardButtonRef} className="buy-btn">
                          買う
                        </button>
                      </div>
                      // )
                    }
                  </>
                )}

                {method === "bank" && (
                  <div className="payment-description">
                    {planData.bankContent1 && (
                      <div
                        dangerouslySetInnerHTML={{
                          __html: planData.bankContent1,
                        }}
                      />
                    )}

                    {planData.bankContent2 && <YuchoVideoModal />}

                    {planData.bankContent2 && (
                      <div
                        dangerouslySetInnerHTML={{
                          __html: planData.bankContent2,
                        }}
                      />
                    )}
                  </div>
                )}

                {method === "paypal" && (
                  <div
                    className="payment-description"
                    dangerouslySetInnerHTML={{
                      __html: planData.paypalContent,
                    }}
                  ></div>
                )}
              </div>
            </div>
          </div>
        </div>
      </div>

      {showModal &&
        (isGroupA ? (
          <div className="modal-overlay">
            <div className="modal-box">
              <h3 className="text-center text-green-color title mt-2 mb-3 fw-bold">
                Credit Card Payment Agreement
              </h3>
              <div className="modal-body" style={{ fontSize: "22px" }}>
                By reviewing the following information and clicking the “Agree”
                button below, you are deemed to have agreed to our
                <span>
                  <a href="/terms-of-service" className="text-decoration-none">
                    【利用規約】
                  </a>
                </span>
                and
                <span>
                  <a href="/privacy-policy" className="text-decoration-none">
                    【個人情報保護方針】.
                  </a>
                </span>
                <br />
                You authorize 日本語の森 to charge your credit card for this
                purchase.
                <br />
                You acknowledge and accept all terms and conditions related to
                this purchase.
                <br />
                This purchase is non-refundable under any circumstances.
              </div>

              <div className="modal-buttons">
                <button
                  className="btn-cancel"
                  onClick={() => {
                    window.location.href = "/plan";
                  }}
                >
                  Cancel
                </button>

                <button
                  className="btn-ok"
                  onClick={() => {
                    setAgreed(true);
                    setShowModal(false);
                  }}
                >
                  Agree
                </button>
              </div>
            </div>
          </div>
        ) : isGroupB ? (
          <div className="modal-overlay">
            <div className="modal-box">
              <h3 className="text-center text-green-color title mt-2 mb-3 fw-bold">
                信用卡付款协议
              </h3>
              <div className="modal-body" style={{ fontSize: "22px" }}>
                在查看以下信息并点击下方的 “同意” 按钮后，
                <br />
                您即被视为已同意我们的
                <span>
                  <a href="/terms-of-service" className="text-decoration-none">
                    【利用規約】
                  </a>
                </span>
                和<br />
                <span>
                  <a href="/privacy-policy" className="text-decoration-none">
                    【個人情報保護方針】.
                  </a>
                </span>
                <br />
                您授权 日本语の森 从您的信用卡中扣除本次购买的费用。
                <br />
                您确认并接受与本次购买相关的所有条款和条件。
                <br />
                本次购买在任何情况下均不予退款。
              </div>

              <div className="modal-buttons">
                <button
                  className="btn-cancel"
                  onClick={() => {
                    window.location.href = "/plan";
                  }}
                >
                  取消
                </button>

                <button
                  className="btn-ok"
                  onClick={() => {
                    setAgreed(true);
                    setShowModal(false);
                  }}
                >
                  同意
                </button>
              </div>
            </div>
          </div>
        ) : isGroupC ? (
          <div className="modal-overlay">
            <div className="modal-box">
              <h3 className="text-center text-green-color title mt-2 mb-3 fw-bold">
                신용카드 결제 동의서
              </h3>
              <div className="modal-body" style={{ fontSize: "22px" }}>
                아래 정보를 확인하고 “동의” 버튼을 클릭하면, 귀하는 당사의
                <br />
                <span>
                  <a href="/terms-of-service" className="text-decoration-none">
                    【利用規約】
                  </a>
                </span>
                및
                <span>
                  <a href="/privacy-policy" className="text-decoration-none">
                    【個人情報保護方針】
                  </a>
                </span>
                에 동의한 것으로 간주됩니다.
                <br />
                귀하는 日本語の森이 이번 구매를 위해 귀하의 신용카드에서
                결제하도록 승인합니다.
                <br />
                귀하는 이번 구매와 관련된 모든 약관을 확인하고 동의합니다.
                <br />
                이번 구매는 어떤 경우에도 환불되지 않습니다.
              </div>

              <div className="modal-buttons">
                <button
                  className="btn-cancel"
                  onClick={() => {
                    window.location.href = "/plan";
                  }}
                >
                  취소
                </button>

                <button
                  className="btn-ok"
                  onClick={() => {
                    setAgreed(true);
                    setShowModal(false);
                  }}
                >
                  동의
                </button>
              </div>
            </div>
          </div>
        ) : (
          <div className="modal-overlay">
            <div className="modal-box">
              <h3 className="text-center text-green-color title mt-2 mb-2">
                ご購入前の同意事項
              </h3>
              <div className="modal-body">
                以下の内容をご確認のうえ、下の「同意する」ボタンを押すことで、
                <br />
                弊社の
                <span>
                  <a href="/terms-of-service" className="text-decoration-none">
                    【利用規約】
                  </a>
                </span>
                および
                <span>
                  <a href="/privacy-policy" className="text-decoration-none">
                    【個人情報保護方針】
                  </a>
                </span>
                に同意したものとみなされます。
                <br />
                <br />
                ※本プランは自動更新のサブスクリプションサービスです。契約期間が終わったら、
                <br />
                契約は自動更新されます。契約を更新したくない場合は、右上の
                <span className="text-green-color">【プランの管理】</span>
                <br />
                ボタンをおしてください。
                <br />
                <br />
                <div className="d-flex justify-content-center">
                  <img
                    src="/images/image_alert_sub.jpg"
                    alt="Alert_Image"
                    className="w-50"
                  />
                </div>
                <br />
                いかなる理由があっても、ご購入後の返金は承っておりませんので、
                <br />
                あらかじめご了承ください。
              </div>

              <div className="modal-buttons">
                <button
                  className="btn-cancel"
                  onClick={() => {
                    window.location.href = "/plan";
                  }}
                >
                  同意しない
                </button>

                <button
                  className="btn-ok"
                  onClick={() => {
                    setAgreed(true);
                    setShowModal(false);
                  }}
                >
                  同意する
                </button>
              </div>
            </div>
          </div>
        ))}
    </>
  );
}

export default Payment;
